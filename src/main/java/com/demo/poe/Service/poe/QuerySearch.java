...
import com.demo.poe.Model.POE.Json.Filters.Options;
import com.demo.poe.Model.POE.Json.Stats.Entries;
import com.demo.poe.Model.POE.Json.Stats.Result;
...
public class QuerySearch {
    ...
    public QuerySearch(VBox mods, TextField itemLevelField, TextField itemQualityField, CheckBox isCorrupted) {
        this.mods = mods;
    ...
    public String createQuery(Map<String, String> item) {
        ...
        List<Mod> modsWithId = combined.stream()
                .flatMap(mod -> StatsRespons.getInstance().getResult().stream()
        ...
        if((itemLevelField.getText() != null && !itemLevelField.getText().isEmpty()) ||
            ...
            if(!itemClass.isEmpty()){
                ...
                query.append("\"category\":{\"option\":\"")
                        .append(FiltersRespons.getInstance().getResult().stream()
    ...
    public Stream<Mod> findIdsByText(List<Entries> entries, Mod mod) {
        return entries.stream()
                .filter(entry -> {
                    if (entry.getId().contains("explicit") && exclusionsItem(entry) && findModInVBox(mod.getName())) {
    ...
    public boolean exclusionsItem(Entries entry) {
